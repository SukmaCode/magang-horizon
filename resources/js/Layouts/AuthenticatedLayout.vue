    ...
    <div class="md:ml-64 flex flex-col h-screen">
      ...
      <header class="bg-surface shadow-sm z-10 shrink-0">
        <div class="px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div class="flex items-center gap-4">
            <div class="flex items-center gap-4">
              <svg class="w-6 h-6" fill="none" stroke="#8B0000" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
              </svg>
          </div>
            <div class="hidden sm:flex flex-col items-end">
              <span class="text-sm font-semibold text-text-primary font-jakarta">
                {{ user.nama_lengkap || user.nama_perusahaan }} 
                <span v-if="user.prodi || user.kontak_person">|</span>
                {{ user.prodi || user.kontak_person }}</span>
              <span class="text-xs font-medium text-primary capitalize">{{ userRoleLabel }}</span>
            </div>
