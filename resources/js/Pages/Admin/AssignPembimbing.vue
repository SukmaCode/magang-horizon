<template>
    <AuthenticatedLayout>
        ...
        <div class="mb-8">
            <h1 class="text-xl font-bold text-text-primary font-jakarta">Assign Dosen Pembimbing</h1>
            ...
            <div class="lg:col-span-2 space-y-6">
                <CardContainer>
                    <div class="py-1 border-b border-gray-100">
                        <h2 class="text-base font-bold text-text-primary font-jakarta">Tugaskan Dosen Pembimbing</h2>
                    </div>
                    
                    <form @submit.prevent="submitAssign" class="p-4">
                        <div class="mb-6">
                            <label class="block text-sm font-semibold text-text-primary mb-2">1. Pilih Dosen Pembimbing <span class="text-danger">*</span></label>
                        ...
                        <div class="mb-6">
                            <div class="flex justify-between items-center mb-2">
                                <label class="block text-sm font-semibold text-text-primary">2. Pilih Mahasiswa Magang <span class="text-danger">*</span></label>
                                <span class="text-xs font-semibold text-primary bg-primary/10 px-2 py-1 rounded-md">{{ form.magang_ids.length }} Dipilih</span>
                            </div>
                            ...
                            <div class="max-h-80 overflow-y-auto border border-gray-200 rounded-xl divide-y divide-gray-100">
                                ...
                                <label v-for="m in magangs" :key="m.id" class="flex items-center gap-3 p-4 hover:bg-gray-50 cursor-pointer transition-colors">
                                    ...
                                    <div>
                                        <p class="text-sm font-bold text-text-primary">{{ m.mahasiswa }}</p>
            ...
            <div class="lg:col-span-1">
                <CardContainer class="h-full">
                    <div class="p-1 border-b border-gray-100">
                        <h2 class="text-md font-bold text-text-primary font-jakarta">Daftar Penugasan Terbaru</h2>
                    </div>
                    <div>
                        <div v-if="assignedMagangs.length > 0" class="space-y-4">
                            <div v-for="am in assignedMagangs" :key="am.id" class="flex flex-col gap-1 p-3 bg-gray-50 rounded-lg border border-gray-100">
                                <p class="text-sm font-semibold text-text-primary">{{ am.mahasiswa }}</p>
                                <p class="text-xs text-text-secondary flex items-center gap-1">
                                    Pembimbing: <span class="font-medium text-text-primary">{{ am.dosen }}</span>
