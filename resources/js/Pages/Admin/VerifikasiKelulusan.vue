            ...
            <div class="lg:col-span-2">
                <CardContainer class="overflow-hidden h-full">
                    <div class="py-2 border-b border-gray-100">
                        <h2 class="text-base font-bold text-text-primary font-jakarta">Menunggu Penerbitan Sertifikat</h2>
                    </div>
                            ...
                            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                <div class="flex-1">
                                    <div class="flex items-center gap-3 mb-1">
                                        <h3 class="text-base font-bold text-text-primary">{{ magang.mahasiswa }}</h3>
                                        <span class="px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-success/10 text-success rounded">Telah Disetujui Prodi</span>
                                    </div>
                                    ...
                                    <div class="mt-2 inline-flex items-center gap-2 text-sm">
                                        ...
                                        <span class="font-bold text-text-primary">{{ magang.nilai_akhir }}</span>
                                    </div>
                                </div>

                                <div class="shrink-0">
                                    <form @submit.prevent="terbitkan(magang.id)">
                                        <button type="submit" class="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-white bg-primary rounded-xl hover:bg-primary-hover transition-colors shadow-sm">
            ...
            <div class="lg:col-span-1">
                <CardContainer class="h-full">
                    <div class="px-6 py-5 border-b border-gray-100">
                        <h2 class="text-base font-bold text-text-primary font-jakarta">Sertifikat Terbaru</h2>
                    </div>
                    <div class="p-4">
                        <div v-if="sertifikats.length > 0" class="space-y-4">
                            <div v-for="cert in sertifikats" :key="cert.id" class="p-4 border border-gray-100 rounded-xl bg-gray-50/50 hover:bg-gray-50 transition-colors cursor-default">
                                <div class="flex items-center gap-3 mb-2">
                                    ...
                                    <div class="min-w-0">
                                        <p class="text-xs font-bold text-text-primary truncate">{{ cert.nomor }}</p>
                                        <p class="text-[10px] text-text-secondary">{{ cert.tanggal }}</p>
                                    </div>
                                </div>
                                <p class="text-sm font-medium text-text-primary pl-11">{{ cert.mahasiswa }}</p>
                            </div>
                        </div>
