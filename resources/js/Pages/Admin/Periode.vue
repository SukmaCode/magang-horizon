<template>
    <AuthenticatedLayout>
        ...
        <div class="mb-8 flex justify-between items-center">
            <div>
                <h1 class="text-lg font-bold text-text-primary font-jakarta">Kelola Periode Magang</h1>
                <p class="text-xs text-text-secondary mt-1">Atur jadwal buka dan tutup pendaftaran magang untuk mahasiswa.</p>
            </div>
            <button @click="showAddModal = true" class="px-3 py-1.5 md:py-2.5 text-xs font-semibold text-white bg-primary rounded-md hover:bg-primary-hover shadow-sm flex items-center gap-2">
                Tambah Periode
            </button>
        </div>
        ...
        <CardContainer class="overflow-hidden">
            <div class="py-1 border-b border-gray-100">
                <h2 class="text-base font-bold text-text-primary font-jakarta">Daftar Periode</h2>
            </div>
                ...
                <table class="w-full text-left text-sm">
                    ...
                    <tbody class="divide-y divide-gray-50" v-if="periodes.length > 0">
                        <tr v-for="periode in periodes" :key="periode.id" class="hover:bg-gray-50/30 transition-colors">
                            <td class="px-6 py-4 font-bold text-text-primary">{{ periode.tahun_akademik }}</td>
                            ...
                            <td class="px-6 py-4">
                                <span v-if="periode.is_active" class="px-2.5 py-1 text-xs font-semibold rounded-full bg-success/10 text-success">
                                    Aktif
                                </span>
                                <span v-else class="px-2.5 py-1 text-xs font-medium rounded-full bg-gray-100 text-gray-600">
                                    Tidak Aktif
                                </span>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <button v-if="!periode.is_active" @click="activatePeriode(periode.id)" class="text-sm font-semibold text-primary hover:text-primary-hover">
                                    Aktifkan
                                </button>
            ...
            <div v-if="showAddModal" class="fixed inset-0 bg-gray-900/60 z-50 flex items-center justify-center p-4" @click.self="showAddModal = false">
                <div class="bg-card rounded-2xl shadow-2xl w-full max-w-md">
                    <div class="px-6 py-5 border-b border-gray-100 flex justify-between items-center">
                        <h3 class="text-lg font-bold text-text-primary font-jakarta">Tambah Periode Baru</h3>
                    ...
                    <form @submit.prevent="submitForm" class="p-6 space-y-4">
                        <div>
                            <label class="block text-sm font-semibold text-text-primary mb-1">Tahun Akademik</label>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-text-primary mb-1">Semester</label>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-semibold text-text-primary mb-1">Tanggal Buka</label>
                            ...
                            <div>
                                <label class="block text-sm font-semibold text-text-primary mb-1">Tanggal Tutup</label>
                        ...
                        <div class="flex gap-3 mt-6 pt-4 border-t border-gray-100">
                            <button type="button" @click="showAddModal = false" class="flex-1 px-4 py-2.5 text-sm font-medium border border-gray-200 rounded-xl hover:bg-gray-50">Batal</button>
                            <button type="submit" :disabled="form.processing" class="flex-1 px-4 py-2.5 text-sm font-semibold text-white bg-primary rounded-xl hover:bg-primary-hover flex justify-center items-center gap-2">
