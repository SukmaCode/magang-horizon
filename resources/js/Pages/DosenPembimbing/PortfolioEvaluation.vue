<template>
    <AuthenticatedLayout>
        ...
        <div class="mb-8">
            ...
            <p class="text-sm font-jakarta text-text-secondary mt-1">Evaluasi portfolio mahasiswa magang bimbingan Anda.</p>
        </div>
        <div v-if="!magangs || magangs.length === 0" class="bg-card rounded-md border border-gray-100 p-12 text-center">
            ...
            <p class="text-xs font-jakarta text-text-secondary">Tidak ada mahasiswa bimbingan yang dapat dievaluasi.</p>
        </div>
        <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            <div v-for="m in magangs" :key="m.id" class="bg-card rounded-md border border-gray-100 hover:border-primary/30 hover:shadow-sm transition-all duration-200 p-5">
...
defineProps({ magangs: { type: Array, default: () => [] } });
