<template>
    <AuthenticatedLayout>
        <Head title="Verifikasi Declaration of Originality" />

        <!-- Header -->
        <div class="mb-8">
            <h1 class="text-xl font-jakartaSemiBold text-text-primary">Verifikasi Declaration of Originality</h1>
            <p class="text-sm font-jakarta text-text-secondary mt-1">Verifikasi dokumen pernyataan keaslian dari seluruh mahasiswa.</p>
        </div>

        <!-- Empty State -->
        <div v-if="!declarations || declarations.length === 0" class="bg-card rounded-md border border-gray-100 p-12 text-center">
            ...
            <p class="text-xs font-jakarta text-text-secondary">Belum ada mahasiswa yang mengupload Declaration of Originality.</p>
        </div>
            ...
            <div class="xl:col-span-1">
                <CardContainer padding="p-0">
                    <div class="px-5 py-4 border-b border-gray-100">
                        <h2 class="text-sm font-jakartaBold text-text-primary">Daftar Dokumen</h2>
                        <p class="text-xs font-jakarta text-text-secondary mt-0.5">{{ declarations.length }} dokumen</p>
                    </div>
                    <div class="divide-y divide-gray-50 max-h-[600px] overflow-y-auto">
                        <button
                            v-for="decl in declarations"
                            :key="decl.id"
                            @click="selectDeclaration(decl)"
                            :class="[
                                'w-full text-left px-5 py-4 transition-colors duration-150 hover:bg-gray-50/80',
                                selected?.id === decl.id ? 'bg-primary/5 border-l-2 border-l-primary' : ''
                            ]"
                        >
                            <div class="flex items-start justify-between gap-2">
                                <div class="min-w-0">
                                    <p class="text-sm font-jakartaSemiBold text-text-primary truncate">{{ decl.mahasiswa.nama_lengkap }}</p>
                                    <p class="text-xs font-jakarta text-text-secondary mt-0.5">{{ decl.mahasiswa.nim }}</p>
                                    <p class="text-[10px] font-jakarta text-text-secondary mt-1">{{ decl.uploaded_at }}</p>
                                </div>
                                <span
                                    :class="getBadgeClass(decl.status)"
                                    class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-jakartaSemiBold shrink-0"
                                >
                                    <span class="w-1 h-1 rounded-full" :class="getDotClass(decl.status)"></span>
                                    {{ decl.status_label }}
                                </span>
                            </div>
                        </button>
        ...
        >
            <div v-if="showRejectModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                <div class="relative bg-white rounded-lg shadow-xl w-full max-w-md p-6 z-10">
                    <h3 class="text-base font-jakartaBold text-text-primary mb-1">Tolak Declaration</h3>
                    <p class="text-xs font-jakarta text-text-secondary mb-4">Berikan catatan revisi atau alasan penolakan.</p>

                    <form @submit.prevent="rejectDeclaration">
                        <textarea
