<template>
    <AuthenticatedLayout>
        ...
        <div class="mb-8">
            ...
            <p class="text-sm font-jakarta text-text-secondary mt-1">Verifikasi dokumen clearance dari perusahaan untuk seluruh mahasiswa.</p>
        </div>

        <!-- Empty State -->
        <div v-if="!clearances || clearances.length === 0" class="bg-card rounded-md border border-gray-100 p-12 text-center">
            ...
            <div class="xl:col-span-1">
                <CardContainer padding="p-0">
                    <div class="px-5 py-4 border-b border-gray-100">
                        ...
                        <p class="text-xs font-jakarta text-text-secondary mt-0.5">{{ clearances.length }} dokumen</p>
                    </div>
                    <div class="divide-y divide-gray-50 max-h-[600px] overflow-y-auto">
                        ...
                        >
                            <div class="flex items-start justify-between gap-2">
                                <div class="min-w-0">
                                    <p class="text-sm font-jakartaSemiBold text-text-primary truncate">{{ cl.mahasiswa.nama_lengkap }}</p>
                                ...
                                >
                                    <span class="w-1 h-1 rounded-full" :class="getDotClass(cl.status)"></span>
                                    {{ cl.status_label }}
                                </span>
                            </div>
                        </button>
...
const downloadUrl = computed(() => {
    ...
    return `${routePrefix}/clearance/${selected.value.id}/download`;
});

const previewUrl = computed(() => {
    if (!selected.value) return '';
    return `${routePrefix}/clearance/${selected.value.id}/preview`;
});
...
function approveClearance() {
    ...
    approveForm.post(`${routePrefix}/clearance/${selected.value.id}/approve`, {
        preserveScroll: true,
        onSuccess: () => {
...
function rejectClearance() {
    ...
    rejectForm.post(`${routePrefix}/clearance/${selected.value.id}/reject`, {
        preserveScroll: true,
        onSuccess: () => {
