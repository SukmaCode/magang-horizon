<template>
    <AuthenticatedLayout>
        <Head title="Declaration of Originality" />

        <!-- Header -->
        <div class="mb-8">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 class="text-xl font-jakartaSemiBold text-text-primary">Declaration of Originality</h1>
                    <p class="text-sm font-jakarta text-text-secondary mt-1">Upload dokumen pernyataan keaslian laporan magang Anda.</p>
                </div>
                <div v-if="declaration" class="flex items-center gap-2">
                    <span
        ...
        <div v-if="!hasMagang" class="bg-warning/5 border border-warning/20 rounded-md p-6 text-center">
            ...
            <p class="text-xs font-jakarta text-text-secondary">Anda harus memiliki magang aktif sebelum dapat mengupload Declaration of Originality.</p>
        </div>
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <CardContainer>
                    <h2 class="text-base font-jakartaBold text-text-primary mb-4">
                        <template v-if="!declaration">Upload Dokumen</template>
                        <template v-else-if="declaration.can_update">Perbarui Dokumen</template>
                        <template v-else>Dokumen Terupload</template>
                    </h2>

                    <!-- Upload Form -->
                    <form v-if="!declaration || declaration.can_update" @submit.prevent="submitFile">
                        <div
                        ...
                        >
                            ...
                            <div v-if="selectedFile" class="flex flex-col items-center">
                                ...
                                <button type="button" @click.stop="removeFile" class="mt-3 text-xs font-jakartaSemiBold text-danger hover:text-danger/80">
                                    Batal
                                </button>
                            </div>
                        ...
                        >
                            ...
                            <template v-else>
                                {{ declaration ? 'Perbarui Dokumen' : 'Upload Dokumen' }}
                            </template>
                        </button>
                    </form>
                    ...
                    <div v-else class="text-center py-4">
                        ...
                        <p class="text-xs font-jakarta text-text-secondary mt-1">Dokumen tidak dapat diubah setelah disetujui.</p>
                    </div>
                </CardContainer>

                <!-- Document Info -->
                <CardContainer v-if="declaration">
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center justify-between gap-2">
                            ...
                            <span :title="declaration.original_filename" class="text-xs font-jakartaSemiBold text-text-primary text-right break-all line-clamp-1">{{ declaration.original_filename }}</span>
                        </div>
                        <div class="border-t border-gray-100"></div>
                        <div class="flex items-center justify-between gap-2">
                            <span class="text-xs font-jakarta text-text-secondary">Tanggal Upload</span>
                            <span class="text-xs font-jakartaSemiBold text-text-primary text-right">{{ declaration.uploaded_at }}</span>
                        </div>
                        <template v-if="declaration.reviewer_name">
                            <div class="border-t border-gray-100"></div>
                            <div class="flex items-center justify-between gap-2">
                                <span class="text-xs font-jakarta text-text-secondary">Diverifikasi Oleh</span>
                                <span class="text-xs font-jakartaSemiBold text-text-primary text-right">{{ declaration.reviewer_name }}</span>
                            </div>
                            <div class="border-t border-gray-100"></div>
                            <div class="flex items-center justify-between gap-2">
                                <span class="text-xs font-jakarta text-text-secondary">Tanggal Verifikasi</span>
                                <span class="text-xs font-jakartaSemiBold text-text-primary text-right">{{ declaration.reviewed_at }}</span>
                            </div>
                        </template>
                    </div>
                </CardContainer>
                ...
                <!-- Rejection Note -->
                <div v-if="declaration && declaration.status === 'rejected' && declaration.rejection_note" class="bg-danger/5 border border-danger/15 rounded-md p-5">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs font-jakarta text-text-secondary mt-1.5 leading-relaxed">{{ declaration.rejection_note }}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <ul class="text-xs font-jakarta text-text-secondary mt-2 space-y-1.5 list-disc list-outside ml-3">
                                <li>Declaration of Originality wajib diupload sebelum menyelesaikan proses magang.</li>
                                <li>Dokumen akan diverifikasi oleh Dosen Pembimbing atau Dosen Prodi.</li>
                                <li>Anda dapat mengganti file selama status masih <strong>Menunggu Verifikasi</strong> atau <strong>Ditolak</strong>.</li>
                                <li>Dokumen yang sudah <strong>Disetujui</strong> tidak dapat diubah.</li>
                            </ul>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-card rounded-md border border-gray-100 overflow-hidden h-[600px] flex flex-col">
                    ...
                    <div class="flex-1 bg-gray-100 relative">
                        ...
                        <div v-else class="absolute inset-0 flex flex-col items-center justify-center text-center p-6">
                            ...
                            <p class="text-xs text-text-secondary">Silakan upload Declaration of Originality melalui form di samping.</p>
                        </div>
                    </div>
                </div>
            </div>
...
// Watch for file selection to generate local preview
...
onMounted(() => {
    if (props.declaration && props.pdfBase64) {
...
function submitFile() {
    const url = props.declaration
        ? `/mahasiswa/declaration/${props.declaration.id}/update`
        : '/mahasiswa/declaration';
